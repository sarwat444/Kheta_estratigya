...
@push('title',__('instructor-dashboard.create video lesson'))
...
@section('content')
    ...
    <div class="dashboard-settings">
        ...
        <div id="upload-form-content">
            <form id="store-lesson-video" action="{{route('dashboard.instructors.lessons.store.video')}}" method="POST">
                @csrf
                <div class="row gy-6">
                    <div class="col-12">
                        <div class="dashboard-content-box">
                            <div class="row gy-4">
                                    ...
                                    <div class="dashboard-content__input">
                                        ...
                                        <select name="course_id" class="form-select select2 course" required>
                                            ...
                                            @foreach($courses as $course)
                                                <option value="{{ $course->id }}"{{ $course->id == request()->input('course_id') ? 'selected' : '' }}>{{ $course->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="dashboard-content__input">
                                        ...
                                        <select name="type" class="form-select select2" required>
                                            ...
                                            <option
                                                value="{{ \App\Enums\LessonType::video->value }}">{{ \App\Enums\LessonType::video->name }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="dashboard-content__input">
                                        ...
                                        <select name="provider" class="form-select select2" required>
                                            ...
                                            <option
                                                value="{{ \App\Enums\LessonProvider::vimeo->value }}">{{ \App\Enums\LessonProvider::vimeo->name }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                ...
                                <div class="col-md-12">
                                    <div class="dashboard-content__input">
                                        <label for="video" class="form-label-02">{{__('instructor-dashboard.Lesson Video')}}</label>
                ...
                <div class="dashboard-settings__btn">
                    <button id="submit-button" class="btn btn-primary btn-hover-secondary section-btn">{{__('instructor-dashboard.Create')}}</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
    ...
    @include('instructors.lessons.scripts.upload-tus-video-lesson')
    @include('instructors.lessons.scripts.course-sections')
    @include('instructors.lessons.scripts.store-video')
@endpush
