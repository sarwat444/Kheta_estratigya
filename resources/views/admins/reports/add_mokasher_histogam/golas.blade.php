    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18"> تقرير  متابعه أداء المؤشرات - تطور الأداء - الأهداف  </h4>
        ...
        @if(!empty($goals))
            @foreach ($goals  as $goal)
                <div class="col-sm-3">
                    <div class="card">
                        <div class="card-body">
                            <h3 style="font-size: 14px;margin-bottom: 30px;"><a href="{{route('dashboard.Histogram_program_statastics',['kheta_id' => $kheta_id ,'goal_id' => $goal->id ])}}">{{ $goal->goal }}</a> </h3>
                            <div id="year_chart{{$goal->id}}"></div>
                        </div>
                    </div>
                </div>
                @php
                    $mokashers_years = \App\Models\MokasherGehaInput::with(['mokasher', 'ex_year', 'mokasher.mokasher_execution_years'])
                    ->whereHas('mokasher.program.goal', function ($query) use ($goal) {
                    ...
                    @foreach($mokashers_years as $mokashers_year)
                        @if($mokashers_year->year_id == $year->id)
                            @php
                                $found = true;
                                $color = '';
                                if (round($mokashers_year->percentage) / $goal->programs_count < 50) {
                                    $color = '#f00';
                                } elseif (round($mokashers_year->percentage) / $goal->programs_count  >= 50 && round($mokashers_year->percentage)  / $goal->programs_count  < 100) {
                                    $color = '#f8de26';
                                } elseif (round($mokashers_year->percentage)  / $goal->programs_count  == 100) {
                                    $color = '#00ff00';
                                }
                                $colors[] = $color;
                            @endphp
                ...
                <script>
                        ...
                        series: [{
                            data: [
                                @foreach($Execution_years as $year)
                                    ...
                                    @foreach($mokashers_years as $mokashers_year)
                                    @if($mokashers_year->year_id == $year->id)
                                    {{ round($mokashers_year->percentage) / $goal->programs_count }},
                                @php
                                    $found = true;
                                @endphp
                                    @endif
                    ...
                    var chart = new ApexCharts(document.querySelector("#year_chart{{$goal->id}}"), options);
                    chart.render();
                </script>
    @endforeach
    @endif
